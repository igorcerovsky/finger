...
# ─────── Hold-size contact-point mechanism (short-finger advantage) ───────────

def _hold_size_analysis(athletes: list, f_mag: float, cfg: SimConfig) -> None:
    """
    Mechanism C: distal phalanx length (Ld) vs hold width — the short-finger advantage.

    Physical setup
    ──────────────
    On a crimp hold the force is applied where the hold edge contacts the volar
    pad.  For all finger sizes, that contact sits approximately d_from_tip mm
    from the fingertip (determined by hold width, ~8–15 mm for a comp crimp).

    Because the ABSOLUTE contact depth from the fingertip is fixed by the hold,
    a LONGER distal phalanx places that contact point proportionally MORE DISTAL
    from the DIP joint (larger moment arm) → higher FDP → higher A2.

    This is structurally separate from the Lp+Lm angle advantage (Mechanism B),
    which lowers the required PIP angle.  The two mechanisms pull in opposite
    directions; the decomposition below isolates each.

    Decomposition
    ─────────────
    We build four "chimera" athletes that each isolate one phalanx group:
      • short (Lp=21, Lm=22, Ld=23)          — reference
      • +Lp+Lm only (Lp=31, Lm=28, Ld=23)   — angle advantage only
      • +Ld only    (Lp=21, Lm=22, Ld=29)    — contact-point penalty only
      • +both       (Lp=31, Lm=28, Ld=29)    — realistic long finger

    Hold height is fixed (short finger's DISTAL_MID y).  When Lp+Lm are longer
    the finger reaches the same height with less PIP flex.  When only Ld is
    longer the PIP angle is unchanged (same DIP height) but the contact is
    further from the DIP joint → higher FDP and A2.
    """
    if not _SCIPY:
        print("=== Hold-Size Contact-Point Mechanism ===")
        print("  [scipy not available]")
        return

    from scipy.optimize import brentq as _brentq

    gname, pip0, dip0, d_abs = "half_crimp", 75.0, -20.0, 0.0
    ratio = dip0 / pip0

    ref_l = athletes[0].lengths_mm          # short finger
    long_l = athletes[2].lengths_mm         # full long finger

    # chimera athletes
    chimeras = [
        ("short (Lp/Lm/Ld = 21/22/23)",        (21.0, 22.0, 23.0)),
        ("+Lp+Lm only (31/28/23)",              (31.0, 28.0, 23.0)),
        ("+Ld only   (21/22/29)",               (21.0, 22.0, 29.0)),
        ("+both      (31/28/29)",               (31.0, 28.0, 29.0)),
    ]

    def _dm_y(lengths, pip):
        pose = posture_from_joint_targets(lengths, pip, ratio * pip, d_abs, 0.0, gname)
        return build_tendon_geometry(pose)["DISTAL_MID"][1]

    def _forces(lengths, pip_deg, d_tip_mm):
        dip_deg = ratio * pip_deg
        pose = posture_from_joint_targets(lengths, pip_deg, dip_deg, d_abs, 0.0, gname)
        geo  = build_tendon_geometry(pose)
        dip_pt = geo["DIP"] * 1e-3
        tip_pt = geo["TIP"] * 1e-3
        Ld_mm  = float(np.linalg.norm(tip_pt - dip_pt) * 1000)
        ...
        dip_flex = pose.theta_m - pose.theta_d
        a2  = (fdp + fds) * 2.0 * np.sin(np.deg2rad(0.44 * pip_flex / 2))
        a4  =  fdp         * 2.0 * np.sin(np.deg2rad(
               (0.25 * pip_flex + 0.25 * max(dip_flex, 0.0)) / 2))
        return dict(FDP=fdp, FDS=fds, A2=a2, A4=a4, pip=pip_deg,
                    Ld=Ld_mm, contact_mm=contact_mm)

    def _solve_pip(lengths):
        """PIP angle for this finger to reach the reference hold height."""
        ref_lp, ref_lm = ref_l[0], ref_l[1]
        same_proximal = (lengths[0] == ref_lp and lengths[1] == ref_lm)
        if same_proximal:
            return pip0          # identical DIP height → no angle adjustment needed
        target_y = _dm_y(ref_l, pip0)
        return float(_brentq(lambda p: _dm_y(lengths, p) - target_y, 5.0, 130.0, xtol=0.02))

    D_TIP_MM = 12.0          # typical comp crimp: 12 mm from fingertip
    D_TIP_SCAN = [8, 10, 12, 15, 18]

    print("=== Mechanism C: Hold-Width Contact-Point (short-finger advantage) ===")
    print(f"  Hold: {gname}  |  contact modelled as {D_TIP_MM:.0f} mm from fingertip")
    print("  Short finger defines hold height; longer fingers adjust PIP accordingly.")
    print(f"  {'athlete':32s} {'pip':>5} {'Ld':>5} {'FDP N':>7} {'FDS N':>7} {'A2 N':>7} {'delta_A2 N':>12} {'delta_A2%':>10}  mechanism")
    pip_ref = pip0
    r_ref = _forces(chimeras[0][1], pip_ref, D_TIP_MM)

    for label, lengths in chimeras:
        pip_sol = _solve_pip(lengths)
        r = _forces(lengths, pip_sol, D_TIP_MM)
        if label == chimeras[0][0]:
            print(f"  {label:32s} {pip_sol:5.0f} {r['Ld']:5.0f} {r['FDP']:7.0f} {r['FDS']:7.0f}"
                  f" {r['A2']:7.0f} {'—':>8} {'[ref]':>7}  ← short reference")
        else:
            da2_n = r["A2"] - r_ref["A2"]
            da2_p = 100.0 * da2_n / max(r_ref["A2"], 1e-6)
            mech = ("← angle advantage (lowers A2)" if "Lp+Lm" in label and "Ld" not in label
                    else "← contact penalty (raises A2)" if "+Ld only" in label
                    else "← net: angle wins but Ld dampens it")
            print(f"  {label:32s} {pip_sol:5.0f} {r['Ld']:5.0f} {r['FDP']:7.0f} {r['FDS']:7.0f}"
                  f" {r['A2']:7.0f} {da2_n:+8.0f} {da2_p:+7.0f}%  {mech}")

    print()
    print("  Hold-width sensitivity (half_crimp, short vs realistic long finger):")
    print(f"  {'d_tip mm':>8} {'A2 short':>10} {'A2 long':>9} {'ΔA2 N':>8} {'ΔA2%':>7}  net winner")
    pip_long = _solve_pip(chimeras[3][1])
    for d_tip in D_TIP_SCAN:
        rs = _forces(chimeras[0][1], pip0, d_tip)
        rl = _forces(chimeras[3][1], pip_long, d_tip)
        da2_n = rl["A2"] - rs["A2"]
        da2_p = 100.0 * da2_n / max(rs["A2"], 1e-6)
        winner = "LONG lower A2" if da2_p < 0 else "SHORT lower A2"
        print(f"  {d_tip:8d} {rs['A2']:10.0f} {rl['A2']:9.0f} {da2_n:+8.0f} {da2_p:+7.0f}%  {winner}")

    print()
    print("  Interpretation:")
    print("  The +Ld contact penalty (+25–35% A2) always works against long fingers.")
    print("  The +Lp+Lm angle advantage (−50% A2) always works for long fingers.")
    print("  For REALISTIC athletes (both Ld and Lp+Lm are longer), the angle")
    print("  advantage dominates and long fingers still have lower A2 in isolation.")
    print()
    print("  The observed SHORT-CLIMBER dominance is therefore NOT primarily a")
    print("  single-finger pulley-load effect. The body-size advantage operates")
    print("  through several reinforcing routes:")
    print("    1. Lower absolute body weight → lower grip force required per hold.")
    print("    2. More favourable strength-to-weight ratio (smaller cross-section")
    print("       scales as L^2, weight as L^3 → smaller athletes are relatively")
    print("       stronger per kg at the same training state).")
    print("    3. Reach efficiency on steep terrain: shorter trunk + proportionally")
    print("       longer arms (common in elite climbers) improves hip-to-hold distance.")
    print("    4. On SMALL HOLDS specifically, Ld contact penalty reduces the long-")
    print("       finger angle advantage: the net A2 benefit shrinks from ~50% to ~40%,")
    print("       not enough to flip the advantage but enough to narrow it.")
    print("    5. Gymnasts are short for the same L^2/L^3 strength-to-weight reason.")
...
def main() -> None:
    ...
    # ── Mechanism C: hold-size contact-point (short-finger advantage) ──
    _hold_size_analysis(athletes, avg_f, cfg_main)
